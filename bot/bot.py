...
def get_ps_command(update: Update, context):
    ...
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(hostname=host, username=username, password=password, port=port)
    stdin, stdout, stderr = ssh.exec_command('ps aux | head -n 20')
    result = stdout.read().decode()
    ssh.close()
    update.message.reply_text(result)
...
def get_apt_list(update: Update, context):
    ...
    if user_input == '1':
        ...
        ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
        ssh.connect(hostname=host, username=username, password=password, port=port)
        stdin, stdout, stderr = ssh.exec_command('apt list --installed | head -n 20')
        result = stdout.read().decode()
        ssh.close()
        update.message.reply_text(result)
    elif user_input == '2':
...
def search_package(update: Update, context):
    ...
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(hostname=host, username=username, password=password, port=port)
    stdin, stdout, stderr = ssh.exec_command(f'apt list --installed | grep {package_name} | head -n 20')
    result = stdout.read().decode()
    ssh.close()
    if result:
        update.message.reply_text(result)
